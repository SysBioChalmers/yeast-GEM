...
%Bump version number:
oldModel   = load('../model/mat/yeastGEM.mat');
...
for i = 1:length(diff)
    ...
    if length(diff_i) == 3
        ...
        if strcmp(diff_i{3},'model/xml/yeastGEM.xml')
...
%Store model as .mat:
save('../model/mat/yeastGEM.mat','model');
...
exportToExcelFormat(model,'../model/xlsx/yeastGEM.xlsx');
