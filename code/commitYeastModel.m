function commitYeastModel(model,upDATE,allowNoGrowth,binaryFiles)
% commitYeastModel
%   Release pipeline for the yeast-GEM model — run this before opening a
%   curation PR. NOT a casual save: this function enforces canonical
%   state (minimal media, SBO terms), validates SBML and growth
%   (aerobic and anaerobic), writes the model in .xml, .yml and .txt
%   (and optionally .xlsx and .mat), persists ΔG annotations, and
%   updates README.md with the current model size and date.
%
%   This function does NOT perform `git commit`; it prepares the
%   artifacts so that the next `git commit` captures a coherent
%   release-ready state.
%
% Inputs:
...
%                   needed (default true).
%   allowNoGrowth   (bool, opt) if committing should be allowed whenever the
%                   model cannot grow, returning a warning (default true),
%                   otherwise will error.
%   binaryFiles     (bool, opt) if the model should also be written in
%                   binary formats (.xlsx and .mat) (default false).
%
% Usage: commitYeastModel(model,upDATE,allowNoGrowth,binaryFiles)
