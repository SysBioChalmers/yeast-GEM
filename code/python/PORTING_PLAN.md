...
This PR ships the **Python side only**: the `yeastgem` package, its data
files, and the CI. The MATLAB-side refactor described in phases 2, 3 and
3.5 below — rewriting `sumBioMass`, `scaleBioMass`, `rescalePseudoReaction`,
`changeGAM`, `addSBOterms`, `loadDeltaG`/`saveDeltaG`, `findDuplicatedRxns`,
`curateMetsRxnsGenes` and `loadYeastModel` as shims over RAVEN, and renaming
`saveYeastModel` to `commitYeastModel` — is **deferred to a follow-up PR**.

Those shims call RAVEN helpers that currently exist only on RAVEN's
unreleased `develop3` branch, and are absent from the released 2.11.2 and
2.11.3. Merging them now would break every MATLAB curation script for
anyone on a released RAVEN. The Python half has no such constraint:
`raven-toolbox` 0.3.0 is on PyPI, so it is resolved by a plain version
constraint. The full phases 1-7 history, including the MATLAB shims, is
preserved on the `archive/feat-python-port-phases-1-7` branch.

Consequently the MATLAB functions in `code/` are untouched by this PR, and
`data/conditions/*.yml` plus `data/yeastgem/ids.yml` are consumed by the
Python side only until the follow-up lands.
...
| 3.5. Upstream restructure (raven-toolbox + RAVEN) | **done** | Decision #1 reversed: generic helpers move upstream rather than living locally. Moved to raven-toolbox (on `feat/yeast-gem-shared`): `raven_toolbox.comparison.diff_models` + `DiffReport` (renamed from the local `compare_models`/`ComparisonReport`), `raven_toolbox.annotation.{add_sbo_terms, load_delta_g_csv, save_delta_g_csv}`, `raven_toolbox.conditions.{apply_condition, load_condition, set_reaction_bounds}`. Moved to RAVEN (on `feat/yeast-gem-shared`): `io/readYAML.m`, `core/applyCondition.m`. yeast-GEM now: depends on `raven-toolbox` (git URL pinned to the feature branch), `yeastgem.compare`/`yeastgem.missing_fields`/`yeastgem.conditions` become thin wrappers that configure upstream defaults with yeast-specific data; MATLAB `code/readYAML.m` deleted, `code/applyCondition.m` → `code/applyYeastCondition.m` (handles the `amino_acid_ratio` pre-step then delegates to RAVEN). yeast-GEM uses the legacy `only_last_reaction_for_pseudo=True` flag on the upstream `add_sbo_terms` to stay byte-equivalent during the transition. **46 new raven-toolbox tests + 46 yeast-GEM tests passing.** Verified: all 4 conditions byte-equivalent pre vs post restructure on MATLAB; Python `commit_yeast_model` (now through raven-toolbox) semantically equal to MATLAB `commitYeastModel`. |
| 4. Tier 2 — biomass + conditions in Python | **done (core)** | Biomass subsystem moved upstream as `raven_toolbox.biomass` (`BiomassConfig` + `BiomassComponent` + `sum_biomass` / `scale_biomass` / `rescale_pseudoreaction` / `set_gam`; 19 new tests on synthetic models). yeast-GEM ids.yml gained a `biomass_components` section; `yeastgem.biomass` exposes `sum_biomass`, `scale_biomass`, `rescale_pseudoreaction` (with the yeast `lipid` → backbone+chain aggregation), `set_gam` (auto-locates the NGAM reaction by name), and `change_amino_acid_ratio` (reads `data/physiology/aminoAcid_Bjorkeroth2020.tsv`). `yeastgem.conditions.apply` now handles `amino_acid_ratio` before delegating to upstream; `yeastgem.io.commit_yeast_model` runs the anaerobic growth check on a copy. **Verified** end-to-end on the real model: Python `conditions.apply('anaerobic')` produces SBML semantically equal to MATLAB `applyYeastCondition('anaerobic')`; Python `commit_yeast_model` (with anaerobic check active) produces SBML semantically equal to MATLAB `commitYeastModel`. 54 yeast-GEM tests + 38 new raven-toolbox tests passing. **Deferred:** chemostat sweep + `fit_gam` (analysis/calibration, not part of the commit pipeline; tracked in UPSTREAM_CANDIDATES.md). |
| 5. Tier 3 — test suite | **done** | Ported the four ``code/modelTests/`` routines to ``yeastgem.model_tests``: ``growth`` (Tobias 2013 chemostat R² across 4 conditions), ``essential_genes`` (cobrapy ``single_gene_deletion`` + Stanford KO collection, returns ``EssentialGeneResult`` dataclass with accuracy / sensitivity / specificity / MCC), ``anaerobic_flux_predictions`` (Jouhten 2008 + Frick & Wittmann flux R² + mean relative error), ``plot_anaerobic`` (fermentation-product bar plot), ``find_duplicated_rxns`` (wrapper over the new ``raven_toolbox.manipulation.find_duplicate_reactions``). Stanford ORF lists extracted from ``essentialGenes.m`` to ``data/essentialGenes/{inviable,verified}_orfs.txt`` so both languages read the same source. 7 new yeast-GEM tests + 6 new raven-toolbox tests; full Python suite 61/61 passing. Verified vs MATLAB on the real model (`runPhase5Metrics.m`): growth R² matches at 1e-7; anaerobic flux R² and essential-gene accuracy/MCC match within 5e-3; single 1-gene difference in the essential-gene confusion matrix is a Gurobi/GLPK solver-tolerance borderline at the 1e-6 ratio threshold. |
...
1. **Upstream stance — generic helpers live upstream (revised after phase 3).**
   yeast-GEM depends on RAVEN (MATLAB) and `raven-toolbox` (Python), and
...
MATLAB (Gurobi via RAVEN) and Python (whichever solver optlang resolves
via cobrapy - GLPK in CI, Gurobi locally if gurobipy is installed).
