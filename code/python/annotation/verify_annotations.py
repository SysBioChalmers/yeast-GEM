...
def write_report(report_path: Path, findings, isolated, recurring, groups, scope: str) -> None:
    ...
    lines = [
        ...
        "",
        "Generated by `code/python/annotation/verify_annotations.py` against "
        "MetaNetX's structure-matched reference tables. A snapshot, not "
        "auto-refreshed by CI -- re-run to update after curating a batch of "
        "annotation. See the script's own docstring for what each status means "
        "and how `wrong` is split below.",
        "",
        f"Scope: {scope}.",
        "",
        "## Summary",
    ...
    def _table(rows, header=("kind", "id", "column", "current", "suggested")):
        out = ["| " + " | ".join(header) + " |", "|" + "|".join("---" for _ in header) + "|"]
        for kind, ent, col, _status, old, new in sorted(rows, key=lambda r: (r[2], r[1])):
            out.append(f"| {kind} | {ent} | {col} | {old or '_(none)_'} | {new} |")
        return out
...
def main() -> int:
    ...
    write_report(args.report, findings, isolated, recurring, groups, scope)
