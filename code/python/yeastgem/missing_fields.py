"""Yeast-specific wrappers for raven-python's annotation helpers.

The mechanism for SBO assignment and ΔG side-car CSV persistence lives
in :mod:`raven_python.annotation`. This module configures those helpers
...
def add_sbo_terms(model: cobra.Model) -> cobra.Model:
    ...
    Thin wrapper over :func:`raven_python.annotation.add_sbo_terms`. The
...
def load_delta_g(model: cobra.Model, *,
    ...
    Thin wrapper over :func:`raven_python.annotation.load_delta_g_csv`.
...
def save_delta_g(model: cobra.Model, *,
    ...
    Thin wrapper over :func:`raven_python.annotation.save_delta_g_csv`.
    """
    met_csv = Path(met_csv) if met_csv else _MET_CSV
    rxn_csv = Path(rxn_csv) if rxn_csv else _RXN_CSV
