...
experimental values. Returns the R² across the data points below a
threshold (default 30, mirroring the legacy script's cap).
"""
...
def anaerobic_flux_predictions(
    ...
    *,
    ...
    ----------
    threshold
        Only data points with experimental flux below this threshold
        (in 100 · v_i / v_glc units) are included in the R² and MRE.
        Default 30, matching the legacy MATLAB script.
    plot
    ...
    if mask.sum() < 2:
        raise RuntimeError(
            f"Need ≥2 data points below threshold={threshold} to compute R²; "
            f"got {mask.sum()}."
        )
    r2 = float(np.corrcoef(sim[mask], data[mask])[0, 1] ** 2)
    mre_terms = np.abs(sim[mask] - data[mask]) / np.where(
        data[mask] == 0, np.nan, data[mask]
    )
    ...
    *,
    ...
    if write_output:
        ...
        (out_dir / "anaerobic_flux_predictions.md").write_text(
            f"## Anaerobic flux R² (threshold < {threshold})\n{r2:.4g}\n\n"
            f"Mean relative error: {mre:.4g}\n\n"
