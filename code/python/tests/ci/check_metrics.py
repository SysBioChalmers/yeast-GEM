...
def _findings(flux, exchange, essential) -> str:
    """The measurements behind each metric, as one markdown file.

    The counts in the QC tables link to the entries behind them; a
    validation number should be traceable the same way, rather than being
    a summary a reader has to take on trust. Sections are named after the
    metrics so a value in the comment links straight to what it summarises.

    Where a test already writes its own detail file, this points at it
    instead of copying it: growth.md holds the per-condition growth
    comparison, essentialGenes.tsv the per-gene calls.
    """
    lines = [
        "# Validation findings",
        "",
        ...
        "",
        "Per-condition measured against predicted growth is in "
        "[growth.md](growth.md), regenerated at release.",
        "",
        "## Anaerobic flux median fold error",
        "",
        f"{len(flux.measurements)} measurements, of which "
        f"{flux.n_unpredicted} have no comparable ratio and are excluded "
        "from the median. Fluxes are scaled to 100 &middot; v/v_glucose.",
        "",
        "| Reaction | Measured | Predicted | Fold error |",
        "|---|---:|---:|---:|",
    ]
    # Worst first: the median says the typical prediction is fine, so the
    # rows worth looking at are the ones it averages over.
    def _sort_key(row):
        try:
            return (0, -float(row[3]))
        except ValueError:
            return (-1, row[0])  # unrateable rows first
    lines += [
        "| " + " | ".join(row) + " |"
        for row in sorted(flux.measurements, key=_sort_key)
    ]
    ...
    for _i, row in exchange.results.iterrows():
        lines.append(
            f"| {row['rxnName']} | {row['measured']:g} +/- {row['stdev']:g} "
            f"| {row['predicted']:.4f} | {row['relativeError']:+.3f} |"
        )
    ...
    lines += [
        ...
        "",
        f"{len(essential.tp) + len(essential.tn) + len(essential.fp) + len(essential.fn)}"
        " genes compared. The per-gene calls are in "
        "[essentialGenes.tsv](essentialGenes.tsv).",
        "",
        "| Call | Count |",
        "|---|---:|",
        f"| True non-essential | {len(essential.tp)} |",
        f"| True essential | {len(essential.tn)} |",
        f"| False non-essential | {len(essential.fp)} |",
        f"| False essential | {len(essential.fn)} |",
    ]
    return "\n".join(lines) + "\n"


def compute(model, findings_path: Path | None = None) -> dict[str, float]:
    ...
    if findings_path is not None:
        ...
        findings_path.write_text(
            _findings(flux, exchange, essential), encoding="utf-8"
        )
...
def main() -> int:
    ...
    parser.add_argument("--findings", type=Path, default=None,
                        help="write the measurements behind each metric here")
    ...
    current = compute(model, args.findings)
