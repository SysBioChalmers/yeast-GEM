...
(**raven-toolbox** for Python, **RAVEN** for MATLAB) so yeast-GEM does not
...
The following were extracted from yeast-GEM and are now released
upstream (raven-toolbox 0.3.0 on PyPI; the MATLAB counterparts are on
RAVEN's `develop3`):
