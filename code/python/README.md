...
# Make some changes …
save_yeast_yaml(model)                          # routine curation save
```

For a local `.xml`/`.txt` build (e.g. to test against a tool that needs
SBML), use `commit_yeast_model(model)` separately — most curation never
needs it, since routine PRs only change `model/yeast-GEM.yml` and the
annotation tsvs.
...
code/python/
  yeastgem/                # the package
    io.py                  # load_yeast_yaml + save_yeast_yaml + commit_yeast_model
