...
* **Compatibility:** First of all, make sure that the model still loads and saves cleanly through `loadYeastYaml.m` & `saveYeastYaml.m` (or `load_yeast_yaml`/`save_yeast_yaml` in Python) and that no errors appear — most PRs only touch `model/yeast-GEM.yml` and the annotation tsvs, so this is normally quick to check. Also confirm the `metaData` block's `id`, `name` and `version` fields in `model/yeast-GEM.yml` haven't changed unexpectedly: those become the model's identity (`model id`/`metaid`/`name`) once exported to SBML, and a stray change there would conflict with the version stamp the next release applies. (`model/dependencies.txt` and the `.xml`/`.txt`/`.xlsx`/`.mat` files are release artifacts now — `commitYeastModel`/`commit_yeast_model` regenerates them independently at release time, so a routine PR touching only the yml/tsvs never changes them; there's nothing to check there for an ordinary pull request.)
...
* **Reproducibility:** If there are any added scripts, make sure that if you run them, the model gets updated from how it was in `develop` to how it is in the pull request. For this, you may _locally_ switch to the corresponding branch, replace `model/yeast-GEM.yml` with the copy from `develop` before the changes (`loadYeastYaml`/`load_yeast_yaml` reads that file, not `.xml`, for routine curation), and run the associated curation script — it should end with `saveYeastYaml`/`save_yeast_yaml`, and the resulting `model/yeast-GEM.yml` should match the pull request's. Remember to stash any undesired changes afterwards.
...
`develop` is never merged into `main` by hand: a release cuts `release/X.Y.Z` from `develop`, builds and stamps it, and opens a reviewed pull request into `main` (see [Releasing a new version](#releasing-a-new-version)) — any conflict between the two branches surfaces there, not from routine merges into `develop`. The one place a conflict can still appear is the release workflow's own sync-back pull request (`main` back into `develop`, opened automatically after each release): if `develop` has diverged a lot in `model/yeast-GEM.yml` since the release was cut, resolve it there like any other merge conflict.
