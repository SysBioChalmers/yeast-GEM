...
6. Now you can make your changes locally!
    ...
    * `python code/python/annotation/verify_annotations.py --all` (or `--mets`/`--rxns` with a comma-separated id list) cross-checks `reactions.tsv`/`metabolites.tsv` identifiers against MetaNetX's structure-matched reference tables and reports any that are wrong, missing, or drifted to a deprecated MetaNetX id — plus, independent of structure, any entity whose own columns (e.g. `chebi` and `kegg.compound`) imply different MetaNetX identities from each other (`conflict`). Not run in CI (needs a ~200MB one-time download) — run it yourself after curating a batch of annotation, or before a release. `--fix` applies the safe corrections (missing/drift) directly to the tsvs; it rewrites the whole file with `\n` line endings, so the diff will show every row as touched even though only the fixed cells' values actually changed — check the *content* diff before committing, not the line count. `wrong`/`conflict` need a human decision and are never auto-applied — a real structural difference (e.g. a sugar's ring vs. open-chain tautomer, or a monatomic ion vs. its neutral atom) can register as a mismatch without being a curation mistake, so check the metabolite's MetaNetX reference name/formula/charge (shown alongside the finding) before deciding. Every run also writes a full markdown report (default `data/annotation/verification_report.md`, override with `--report`) with every finding, not just the console preview. `wrong` findings are further split into isolated — a unique suggested correction, worth a look on its own — and recurring — the same correction shared by 3+ entities, grouped into a collapsible section so you can sanity-check the pattern once instead of row by row.
