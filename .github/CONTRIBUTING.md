...
6. Now you can make your changes locally!
    ...
    * Reaction, metabolite and gene cross-reference identifiers (KEGG, BiGG, ChEBI, MetaNetX, EC numbers, UniProt) live in `model/reactions.tsv`, `model/metabolites.tsv` and `model/genes.tsv`, not in `model/yeast-GEM.yml` itself (see [model/README.md](../model/README.md)). Curate those directly by editing the matching column — no RAVEN or cobrapy needed. `loadYeastModel.m` merges them back into the model automatically, so this is transparent for any other kind of curation. A newly added reaction, metabolite or gene needs a row added to the matching tsv too, and CI fails the build if the model and the tsvs disagree about which ids exist. `reactions.tsv`/`metabolites.tsv` also carry a `name` column: the yml stays authoritative, so if you rename a reaction or metabolite, update the tsv row's `name` to match in the same change — CI fails the build otherwise. Removing a reaction or metabolite also needs an entry in `data/deprecatedIdentifiers/deprecatedReactions.tsv` or `deprecatedMetabolites.tsv` (CI reports, but does not block, if this is missed) — and reusing a deprecated id for something new fails the build.
    * `python code/python/annotation/verify_annotations.py --all` (or `--mets`/`--rxns` with a comma-separated id list) cross-checks `reactions.tsv`/`metabolites.tsv` identifiers against MetaNetX's structure-matched reference tables and reports any that are wrong, missing, or drifted to a deprecated MetaNetX id — plus, independent of structure, any entity whose own columns (e.g. `chebi` and `kegg.compound`) imply different MetaNetX identities from each other (`conflict`) — resolved, where possible, using the entity's own structure match or a majority of its other columns, giving a suggested fix and the MetaNetX name/formula behind each column's value; where nothing settles it, those same names/formulas are shown for you to compare by eye. Runs on every pull request (a dedicated `annotation` job in `model-qc.yml`, since it needs a ~200MB MetaNetX download — cached across runs) and reports into the same pull-request comment as the rest of `model-qc.yml`, compared against the target branch's own last-committed summary rather than a fresh run on it, the same way the MEMOTE score is — see `data/testResults/README.md`. Not a gate: `wrong`/`conflict` need a human decision, not a pass/fail rule. You can also run it yourself locally, e.g. scoped to just the metabolites/reactions you are curating with `--mets`/`--rxns`. `--fix` applies the safe corrections (missing/drift) directly to the tsvs; it rewrites the whole file with `\n` line endings, so the diff will show every row as touched even though only the fixed cells' values actually changed — check the *content* diff before committing, not the line count. `wrong`/`conflict` are never auto-applied — a real structural difference (e.g. a sugar's ring vs. open-chain tautomer, or a monatomic ion vs. its neutral atom) can register as a mismatch without being a curation mistake, so check the metabolite's MetaNetX reference name/formula/charge (shown alongside the finding) before deciding. Every run also writes a full markdown report and a `key<TAB>value` summary into `--out` (default `data/testResults`, matching `model_qc.py`) with every finding, not just the console preview. `wrong` findings are further split into isolated — a unique suggested correction, worth a look on its own — and recurring — the same correction shared by 3+ entities, grouped into a collapsible section so you can sanity-check the pattern once instead of row by row.
