...
signature — and separately checks whether an entity's own columns (e.g.
`chebi` and `kegg.compound`) even agree with each other via MetaNetX's id
graph, independent of structure. See the script's own docstring for exactly
how each status below is derived.
...
a regression, for two reasons: `wrong` and `conflict` findings need a human
decision — a ring/open-chain sugar tautomer or a generic-vs-specific
stereoisomer pair is a genuine structural difference, not necessarily a
curation mistake — and MetaNetX's own reference data can shift these counts
on its own, independent of anything a pull request did.
...
MetaNetX's reference name/formula/charge alongside each `wrong`/`conflict`
entry as context for the human decision it needs. Also runnable directly,
e.g. scoped to just the metabolites/reactions a pull request touched with
